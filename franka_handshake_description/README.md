...
2. Install the package by calling `colcon build --symlink-install --packages-select franka_handshake_description`
3. (Re-)Source environment `source install/setup.bash`


> **NOTE:** If you use [RosTeamWorkspace (RTW)](https://rtw.stoglrobotics.de) than instead of the previous three steps, use `cb franka_handshake_description` command.

Now, launch description test:
```
ros2 launch franka_handshake_description view_franka_handshake.launch.xml
```
or
```
ros2 launch franka_handshake_description view_franka_handshake.launch.py
```
