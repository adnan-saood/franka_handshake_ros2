...
COPY franka_ros2/ /ros2_ws/src
# Remove git directory to avoid issues with vcs import
RUN rm -rf /ros2_ws/src/.git
RUN sudo chown -R $USERNAME:$USERNAME /ros2_ws \
    && vcs import src < /ros2_ws/src/franka.repos --recursive --skip-existing \
    && sudo apt-get update \
    && rosdep update \
    && rosdep install --from-paths /ros2_ws/src --ignore-src --rosdistro $ROS_DISTRO -y \
    && sudo apt-get clean \
    && sudo rm -rf /var/lib/apt/lists/* \
...
COPY ./franka_ros2/franka_entrypoint.sh /franka_entrypoint.sh
RUN sudo chmod +x /franka_entrypoint.sh
RUN mkdir -p /home/robot/ws/src
