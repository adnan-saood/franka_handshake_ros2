...
#                   (default: franka.config.yaml in franka_handshake_bringup/config)
#
...
# Example:
# ros2 launch franka_handshake_bringup example.launch.py controller_name:=elbow_example_controller
#
...
# franka_handshake_bringup/config directory. See that file for its own documentation.
#
...
package_share = get_package_share_directory('franka_handshake_bringup')
utils_path = os.path.join(package_share, '..', '..', 'lib', 'franka_handshake_bringup', 'utils')
            ...
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    PathJoinSubstitution([
                        FindPackageShare('franka_handshake_bringup'), 'launch', 'franka.launch.py'
                    ])
                ),
                launch_arguments={
        ...
        nodes.append(
            Node(
                ...
                parameters=[PathJoinSubstitution([
                    FindPackageShare('franka_handshake_bringup'), 'config', "controllers.yaml",
...
# via: ros2 launch franka_handshake_bringup example.launch.py ARGS...
    ...
    return LaunchDescription([
        DeclareLaunchArgument(
            ...
            default_value=PathJoinSubstitution([
                FindPackageShare('franka_handshake_bringup'), 'config', 'franka.config.yaml'
            ]),
            description='Path to the robot configuration file to load',
        ),
        DeclareLaunchArgument(
            'controller_name',
            default_value='handshake_controller',
